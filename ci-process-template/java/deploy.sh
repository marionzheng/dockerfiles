...
if [ $SERVICE_NAME == 'micro-teaching' ];then
  cat yaml/workload.yaml.teaching | sed 's|{env}|'$ENV'|g; s|{service-name}|'$SERVICE_NAME'|g; s|{image-name}|'$IMAGE_NAME'|g; s|{image-tag}|'$IMAGE_TAG'|g; s|{http-port}|'$HTTP_PORT'|g; s|{grpc-port}|'$GRPC_PORT'|g; s|{programming_language}|'$PROGRAMMING_LANGUAGE'|g' > yaml/workload.yaml
else
