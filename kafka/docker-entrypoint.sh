...
sed -i "s#listeners=PLAINTEXT://localhost:9092#listeners=PLAINTEXT://$KAFKA_HOST_NAME:9092#g" $KAFKA_CONF_DIR/server.properties
sed -i "s#advertised.listeners=PLAINTEXT://localhost:9092#advertised.listeners=PLAINTEXT://$KAFKA_HOST_NAME:9092#g" $KAFKA_CONF_DIR/server.properties
