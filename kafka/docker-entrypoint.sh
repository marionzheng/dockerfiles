...
if [[ -x "$KAFKA_BROKER_ID" ]]; then
    sed -i "s/broker\.id=(\d+)/broker.id=$KAFKA_BROKER_ID/g" $KAFKA_HOME/config/server.properties
fi
...
if [[ -x "$KAFKA_HOST_NAME" ]]; then
    sed -i "s/host\.name=(\w+)/host.name=$KAFKA_HOST_NAME/g" $KAFKA_HOME/config/server.properties
fi

if [[ -x "$KAFKA_PORT" ]]; then
    sed -i "s/port=(\d+)/port=$KAFKA_PORT/g" $KAFKA_HOME/config/server.properties
fi

if [[ -x "$KAFKA_ADVERTISED_HOST_NAME" ]]; then
    sed -i "s/advertised\.host\.name=(\w+)/advertised.host.name=$KAFKA_ADVERTISED_HOST_NAME/g" $KAFKA_HOME/config/server.properties
fi

if [[ -x "$KAFKA_ADVERTISED_PORT" ]]; then
    sed -i "s/advertised\.port=(\d+)/advertised.port=$KAFKA_ADVERTISED_PORT/g" $KAFKA_HOME/config/server.properties
fi
